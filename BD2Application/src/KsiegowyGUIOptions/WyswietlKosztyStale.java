...
public class WyswietlKosztyStale {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					WyswietlKosztyStale window = new WyswietlKosztyStale();
	...
	private void initialize() {
		...
		btnWstecz.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				ZarzadzajKosztyStale.NewScreen();
				frame.dispose();
			}
		});
		...
		ResultSet res = Main.db.SqlREAD("SELECT * FROM KOSZTY_STALE");
		try {
			while(res.next())
			{
				model.addRow(new Object[]{res.getInt(1), res.getString(2),res.getInt(3)
						});
			}
