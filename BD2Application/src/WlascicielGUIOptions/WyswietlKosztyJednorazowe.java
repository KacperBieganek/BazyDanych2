...
public class WyswietlKosztyJednorazowe {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					WyswietlKosztyJednorazowe window = new WyswietlKosztyJednorazowe();
	 ...
	 */
	public WyswietlKosztyJednorazowe() {
		initialize();
	}
	...
	private void initialize() {
		...
		ResultSet res = Main.db.SqlREAD("SELECT * FROM KOSZTY_JEDNORAZOWE");
		try {
			while(res.next())
			{
				model.addRow(new Object[]{String.valueOf(res.getInt(1)), res.getString(2),res.getInt(3),
